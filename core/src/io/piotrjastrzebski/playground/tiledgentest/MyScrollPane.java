...
/**
 * SimpleScrollPane that doesnt have scroll bars and doesnt eat scrolled event
...
public class MyScrollPane extends WidgetGroup {
	...
	public MyScrollPane (Actor widget) {
	...
	/** @param widget May be null. */
	public MyScrollPane (Actor widget, Skin skin, String styleName) {
		this(widget, skin.get(styleName, ScrollPaneStyle.class));
	}

	/** @param widget May be null. */
	public MyScrollPane (Actor widget, ScrollPaneStyle style) {
		...
		addCaptureListener(new InputListener() {
			...
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				if (scrollX && hScrollBounds.contains(x, y) && knobXEnabled) {
					event.stop();
					resetFade();
				...
				if (scrollY && vScrollBounds.contains(x, y) && knobYEnabled) {
					event.stop();
					resetFade();
			...
			public void touchDragged (InputEvent event, float x, float y, int pointer) {
				...
				if (touchScrollH && knobXEnabled) {
					...
					lastPoint.set(x, y);
				} else if (touchScrollV && knobYEnabled) {
		...
		addListener(new InputListener() {
			public boolean scrolled (InputEvent event, float x, float y, int amount) {
				if (!mouseScrollEnabled) return false;
				resetFade();
				if (scrollY)
	...
	public void draw (Batch batch, float parentAlpha) {
		...
		if (scrollX && scrollY && knobXEnabled && knobYEnabled) {
...
//		if (scrollY && vScrollBounds.contains(x, y)) return this;
	...
	/**
	 * Enabled or disable drawing and function of knobs */
	public void setKnobsEnabled(boolean knobX, boolean knobY) {
		knobXEnabled = knobX;
		knobYEnabled = knobY;
	}

	/**
	 * When enabled, mouse scroll will scroll the pane in x direction. Default is true. */
