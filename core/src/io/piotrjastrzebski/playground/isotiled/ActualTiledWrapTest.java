 ...
 */
public class ActualTiledWrapTest extends BaseScreen {
	public final static float MAP_WIDTH = 64;
	public final static float MAP_HEIGHT = 48;
	...
	WrapTiledMapRenderer mapRenderer;
	private boolean useShader;

	public ActualTiledWrapTest (GameReset game) {
		super(game);

		TiledMap map = new TmxMapLoader().load("tiled/map.tmx");
		mapRenderer = new WrapTiledMapRenderer(map, INV_SCALE, batch);
...
//		mapRenderer.setView(gameCamera.combined, vb.x, vb.y, vb.width, vb.height);
//		mapRenderer.render();
//		mapRenderer.setOffsets(-vb.width/2, -vb.height/2);
//		mapRenderer.setView(gameCamera.combined, vb.x + vb.width/2, vb.y + vb.height/2, vb.width, vb.height);
		mapRenderer.setOffsets(0, 0);
		mapRenderer.setView(gameCamera.combined, vb.x, vb.y, vb.width, vb.height);
		mapRenderer.render();
		mapRenderer.setOffsets(-MAP_WIDTH, 0);
		mapRenderer.setView(gameCamera.combined, vb.x + MAP_WIDTH, vb.y, vb.width, vb.height);
		mapRenderer.render();
		mapRenderer.setOffsets(0, -MAP_HEIGHT);
		mapRenderer.setView(gameCamera.combined, vb.x, vb.y + MAP_HEIGHT, vb.width, vb.height);
		mapRenderer.render();
		mapRenderer.setOffsets(-MAP_WIDTH, -MAP_HEIGHT);
		mapRenderer.setView(gameCamera.combined, vb.x + MAP_WIDTH, vb.y + MAP_HEIGHT, vb.width, vb.height);
		mapRenderer.render();
		batch.end();
		/*
		Gdx.gl.glClearColor(.6f, .6f, .6f, 1);
...
//			gameCamera.position.x + marginX,
			gameCamera.position.y - VP_HEIGHT / 2 + marginY,
//			gameCamera.position.y + marginY,
			VP_WIDTH - marginX * 2,
			VP_HEIGHT - marginY * 2
		);
	}
	...
	public static void main (String[] args) {
		PlaygroundGame.start(args, ActualTiledWrapTest.class);
	}
}
